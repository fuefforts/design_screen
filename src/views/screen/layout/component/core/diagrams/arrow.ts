import { LineAnimateType, type Pen } from '../pen'
import type { Point } from '../point'

//箭头动画
export function drawArrow(pen: Pen, ctx?: CanvasRenderingContext2D | Path2D) {
  ...
  const d = (pen.animateInterval || 100) * scale // 箭头间距
  const smoothLenth = pen.calculative.lineWidth * (pen.calculative.lineSmooth || 0) //*scale;
  const lineWidth = (pen.calculative.animateLineWidth / 2 || 3) * scale
  ...
  if (worldAnchors.length > 1) {
    ...
    for (let i = 0; i < worldAnchors.length; i++) {
      const pt = worldAnchors[i]
      //获取箭头角度
      if (from) {
        const angle = getAngle(from, pt)
        let newP = {
          x:
            from.x +
            ((pen.calculative.animatePos - lastLength) % d) * Math.cos((angle * Math.PI) / 180),
          y:
            from.y -
            ((pen.calculative.animatePos - lastLength) % d) * Math.sin((angle * Math.PI) / 180)
        }
        if (pen.animateReverse) {
          newP = {
            x:
        ...
        const ptTFrom = Math.sqrt((pt.x - from.x) ** 2 + (pt.y - from.y) ** 2)
...
function getRotatePoint(p: Point, rp: Point, _angle: number) {
  const angle = ((180 - _angle) * Math.PI) / 180
  return {
...
function arrow(
  ...
  const pr = getRotatePoint(
    { x: newP.x + size, y: newP.y + 0.57 * size },
    { x: newP.x, y: newP.y },
    angle
  )
  const pl = getRotatePoint(
    { x: newP.x + size, y: newP.y - 0.57 * size },
    { x: newP.x, y: newP.y },
    angle
  )
  const pr1 = getRotatePoint(
    { x: newP.x + size, y: newP.y + lineWidth / 2 },
    { x: newP.x, y: newP.y },
    angle
  )
  const pr2 = getRotatePoint(
    { x: newP.x + arrowLength, y: newP.y + lineWidth / 2 },
    { x: newP.x, y: newP.y },
    angle
  )
  const pl1 = getRotatePoint(
    { x: newP.x + size, y: newP.y - lineWidth / 2 },
    { x: newP.x, y: newP.y },
    angle
  )
  const pl2 = getRotatePoint(
    { x: newP.x + arrowLength, y: newP.y - lineWidth / 2 },
    { x: newP.x, y: newP.y },
    angle
  )
...
function waterDrop(
  ...
  const pl1 = getRotatePoint({ x: newP.x, y: newP.y + dis }, { x: newP.x, y: newP.y }, angle)
  const pE = getRotatePoint({ x: newP.x + arrowLength, y: newP.y }, { x: newP.x, y: newP.y }, angle)
