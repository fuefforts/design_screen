...
export class CanvasImage {
  ...
  render() {
    ...
    if (patchFlagsBackground && this.isBottom) {
      ...
      const background =
        this.store.data.background || (this.store.bkImg ? undefined : this.store.options.background)
      if (background) {
        ctx.save()
  ...
  renderRule(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D) {
    ...
    ctx.strokeStyle = rgba(finalRuleColor as any, 0.7)

    const x = origin.x + data.x
    const y = origin.y + data.y
    const { width, height } = this.canvas

    const h = options.ruleOptions?.height || 20
    ...
    if (options.ruleOptions?.underline) {
      ctx.beginPath()
      ctx.fillStyle = rgba(finalRuleColor as any, 0.7)
      ctx.moveTo(0, h)
      ctx.lineTo(width, h)
      ctx.stroke()
      ctx.moveTo(h, 0)
    ...
    // the big rule
    ctx.strokeStyle = finalRuleColor as any
    ctx.beginPath()
    ctx.lineWidth = h
    ...
    const textTop = options.ruleOptions?.textTop || 16
    const textLeft = options.ruleOptions?.textLeft || 4
    if (x < 0) {
      text -= 100
    }
