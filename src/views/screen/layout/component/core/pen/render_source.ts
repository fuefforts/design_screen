import { CanvasLayer, ColorStop, IValue, LineAnimateType, LockState, Pen } from './model'
...
function drawUnderLine(ctx: CanvasRenderingContext2D, location: any, config: any) {
...
function drawStrickout(ctx: CanvasRenderingContext2D, location: any, config: any) {
...
export function facePen(pt: Point, pen?: Pen) {
  ...
  if (pt.anchorId) {
    const anchor = pen.anchors.filter((_anchor) => _anchor.id === pt.anchorId)
    if (anchor.length) {
...
export function connectLine(pen: Pen, anchor: Point, line: Pen, lineAnchor: Point) {
  ...
  const i = pen.connectedLines.findIndex(
    (item) =>
      item.lineId === line.id && item.lineAnchor === lineAnchor.id && item.anchor === anchor.id
  )
  ...
  const fromAnchor =
    line.calculative.worldAnchors?.length >= 2
      ? line.calculative.canvas.store.pens[
          line.calculative.worldAnchors?.[0].connectTo
        ]?.anchors.find((i) => i.id === line.calculative.worldAnchors?.[0].anchorId)
      : undefined // num
...
export function setElemPosition(pen: Pen, elem: HTMLElement) {
  ...
  elem.style.display =
    pen.calculative.inView != false ? pen.calculative.cssDisplay || 'inline' : 'none' // 是否隐藏元素
  ...
  if (pen.type) {
    ...
    if (pen.parentId) {
      pen.calculative.worldRect.x =
        pen.calculative.worldRect.center.x - pen.calculative.worldRect.width / 2
      pen.calculative.worldRect.y =
        pen.calculative.worldRect.center.y - pen.calculative.worldRect.height / 2
...
export function setChildValue(pen: Pen, data: IValue) {
  ...
  if (pen.calculative.canvas.parent.isCombine(pen) && pen.showChild === undefined) {
