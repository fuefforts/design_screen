...
function rewritePenLifeCycle() {
  ...
  return (pen: Pen, lifeCycle, func: Function, del = false) => {
    ...
    const originFuncMap = new Map() // 原始事件回调Map
    ...
    const originLifeCycle = originFuncMap.get(lifeCycle) // 原始事件;
    const rewriteFunc = (...args) => {
...
export const setLifeCycleFunc = rewritePenLifeCycle()
