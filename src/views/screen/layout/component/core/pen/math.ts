...
// import { calcCenter, expandRect, Rect, rectInFourAngRect, rectToPoints } from '../rect'
import type { Meta2dStore } from '../store'
import { deepClone } from '../utils'
import type { Pen } from './model'

export function calcAnchorDock(store: Meta2dStore, e: Point, curAnchor?: Point) {
  let xDock: Point
  let yDock: Point
  let x = Infinity
  let y = Infinity
  ...
  for (const pen of store.data.pens) {
    ...
    points.forEach((pt) => {
      ...
      const distance =
        (pen.calculative.worldRect.center.x - e.x) * (pen.calculative.worldRect.center.x - e.x) +
        (pen.calculative.worldRect.center.y - e.y) * (pen.calculative.worldRect.center.y - e.y)
...
export function calcMoveDock(store: Meta2dStore, rect: Rect, pens: Pen[], offset: Point) {
