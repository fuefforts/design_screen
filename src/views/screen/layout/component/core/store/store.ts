...
import { CanvasLayer, type FormItem, LockState, type Pen } from '../pen'
...
export const clearStore = (store: Meta2dStore, template?: string) => {
  ...
  if (isSame) {
    ...
    for (const pen of store.data.pens) {
      // if (pen.template) {
      if (pen.canvasLayer === CanvasLayer.CanvasTemplate && store.templatePens && pen.id) {
  ...
  store.data = {
    ...
    center: { x: 0, y: 0 },
    paths: {},
    template: isSame ? template : (null as any),
    distance: { x: 0, y: 0 }
  }
  store.sameTemplate = isSame
  store.pens = {}
  store.histories = []
  store.historyIndex = null as any
